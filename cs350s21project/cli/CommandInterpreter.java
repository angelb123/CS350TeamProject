...
public class CommandInterpreter  {
	public void evaluate(String command) throws RuntimeException {
		...
		String s = "Define Bomb; Define car;"; // example
		
		String ss[] = seperateCommands(s);
		
		for(int i = 0; i < ss.length; i ++) {
			System.out.println("test: " + "i" + i + ": "+ ss[i]);
		}
	...
	public String[] seperateCommands(String _command) { //Return a list of string commands - Angel
		...
		for(int i = 0; i < _command.length(); i++) {
			if(_command.charAt(i) == ';') {
				String temp = _command.substring(lastIndex, i);
				lastIndex = i+1;
				_commands.add(temp);
			}
		}
...
>>>>>>> branch 'main' of https://github.com/angelb123/CS350TeamProject.git
